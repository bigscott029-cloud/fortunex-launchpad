...
import { PROMO, getPromoState } from "@/config/glamour";

const getPlusRegistrationMessage = (
  promoActive: boolean,
  verb: "Just registered" | "Successfully registered" | "Registered" = "Registered"
) => {
  const promoPrice = PROMO.plusPromoPrice.toLocaleString();
  const regularPrice = PROMO.plusRegularPrice.toLocaleString();
  const promoSuffix =
    verb === "Successfully registered"
      ? ". Grab your opportunity too."
      : verb === "Just registered"
      ? ", join today while the promo is ongoing."
      : ` before it returns to ₦${regularPrice}.`;

  return promoActive
    ? `${verb} for Glamour Plus at ₦${promoPrice}${promoSuffix}`
    : `${verb} for Glamour Plus at ₦${regularPrice}.`;
};

const getPulseItems = (promoActive: boolean) => [
  {
        icon: User,
    title: "John Oluwafemi",
    message: getPlusRegistrationMessage(promoActive, "Successfully registered"),
  },
  {
    icon: User,
  ...
  },
  {
    icon: User,
    title: "Chukwuma Eze",
    message: getPlusRegistrationMessage(promoActive, "Just registered"),
  },
  {
    icon: User,
  ...
  {
    icon: Flame,
    title: promoActive ? "Limited GlamSlots" : "Regular Price Active",
    message: promoActive
      ? `Plus promo access is active at ₦${PROMO.plusPromoPrice.toLocaleString()} while the countdown runs.`
      : `The promo window has closed. Glamour Plus is now ₦${PROMO.plusRegularPrice.toLocaleString()}.`,
  },
  {
    icon: PlayCircle,
    title: "Watch First",
  ...
  {
    icon: Clock3,
    title: promoActive ? "Fixed Promo Timer" : "Promo Can Be Reset",
    message: promoActive
      ? "The offer countdown keeps moving when visitors come back."
      : "Admin can set a new future deadline to restart the promo window.",
  },
  {
    icon: Tag,
    title: promoActive ? "Special Promo" : "Full Access Price",
    message: promoActive
      ? `Register now at ₦${PROMO.plusPromoPrice.toLocaleString()} before it returns to ₦${PROMO.plusRegularPrice.toLocaleString()}.`
      : `Glamour Plus remains available at ₦${PROMO.plusRegularPrice.toLocaleString()} , be a part today.`,
  },
  {
    icon: Package,
    title: "Two Packages",
...
export const SitePulse = ({ className = "" }: SitePulseProps) => {
  ...
  const [promoActive, setPromoActive] = useState(getPromoState().isActive);

  const items = useMemo(() => {
    return getPulseItems(promoActive).sort(() => Math.random() - 0.5);
  }, [promoActive]);
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      setPromoActive(getPromoState().isActive);
